...
- DAG as a TikZ figure (publication-ready, using `\tikz` with plate notation:
  observed nodes shaded, latent nodes open, plates for repeated structure,
  hyperparameters outside plates). Write this as a standalone `.tex` file that
  can be `\input{}` into the paper. Use standard graphical model conventions
  (Bishop 2006 style).
...
4. Repeat ≥100 times to build statistics
5. Check **ensemble** properties — NOT individual mocks:
   - **Coverage:** Do X% credible intervals contain the truth X% of the time
     across all mocks? Plot the P-P curve.
   - **Bias:** Compute the mean bias (posterior mean minus truth) across all
     mocks. This must be consistent with zero. A -1σ bias on a single mock
     is meaningless — what matters is whether the AVERAGE across 100+ mocks
     is biased. If the mean bias is more than ~0.1-0.2σ (scaled by 1/sqrt(N_mocks)),
     the pipeline has a systematic problem.
   - **Calibration:** The posterior uncertainty should match the actual scatter
     of point estimates across mocks. If posteriors are too narrow, the pipeline
     is overconfident. If too wide, it is conservative.

**CRITICAL:** A single mock recovering the truth within 2σ does NOT validate
the pipeline. The question is always: across many independent mocks, is the
average offset from truth consistent with zero? Individual mocks will scatter
— that is expected. Systematic bias is revealed only by the ensemble.
...
- **GL:** Writes the methodology section. Describes the forward model with proper
  Bayesian notation, the likelihood, and priors. Includes the TikZ DAG figure
  from Phase 2 (refine it if the model changed during implementation). Uses
  "plausible" and careful technical language. References prior work where relevant.
...
- TikZ DAG figure of the generative model (standalone `.tex` file)
- Figure descriptions for any additional figures (even if not generated)
